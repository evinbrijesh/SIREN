...
- **South Lhonak real-DEM diagnostic (2026-09-17):** `ml/south_lhonak_lake_eval.py` measures the 2023-10-03 GLOF from the on-disk 1 m Pleiades pre/post DEMs (CC BY 4.0). Flat-surface detection gives pre-event area 0.866 km² (published 0.9–1.2) and 14 m surface drawdown (documented 10–20). Huggel on the measured area → 28.4 MCM vs documented 40–50 MCM release (−36.8%); across the published area range it spans 28–45 MCM, bracketing the truth — the dominant error is the area input, not the exponent. Caveats: DSM differencing only sees drawdown above the post-event waterline (visible emptied volume 0.13 MCM is a strict lower bound), and post-event "flat" pixels include exposed lakebed. The FNO `eval_south_lhonak.py` uses a *synthetic* parametric corridor and clips wave speed into the documented range — its 12.3% MAPE is not real-terrain evidence.
