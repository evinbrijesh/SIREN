...
>
> **Also fixed 2026-09-17:** `detect/sar.py::dem_slope` divided per-pixel gradients by metres-per-degree (~1e5× too small) — every slope gate was silently inert. Calibrated SAR caches now persist S1 GCP geolocation (the measurement TIFFs carry no geotransform; a fitted affine has 1–4 km residuals over mountain terrain, so GCPs + `GCPTransformer` are the honest georeference). New helpers: `sar_grid_lonlat`, `sar_grid_dem_slope`, `sar_grid_polygon_mask`, `sar_grid_sample` in `detect/sar.py`. **`data/processed/baseline_water_mask.tif` covers the Rolwaling valley ~30 km west of the AOI** — it is not a valid Imja lake reference; use the union of `obs-*_expansion_mask.tif` instead.
