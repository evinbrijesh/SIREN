...
2. Three observations process through the pipeline (2026-07-23, 2026-08-04, 2026-09-04)
3. An elevated review card appears with ≥3 evidence reasons
...
docs/
  PRD.md          # Product Requirements Document (v4.2)
...
|---|---|---|
| GET | `/basin` | Active basin configuration |
...
| POST | `/runs/{id}/dispatch` | Dispatch alert (requires prior confirm) |
| GET | `/audit?alert_id={id}` | Audit lineage for an alert |
| GET | `/data/processed/{file}` | Static raster file access |
...
| Observation | Date | Sensor | Expansion | Severity | Story |
|---|---|---|---|---|---|
| Baseline | 2025-11-22 | S2 Optical | — | — | Clear post-monsoon baseline |
| obs-001 | 2026-07-23 | S1 SAR | +8% | Watch | Early warning sign |
| obs-002 | 2026-08-04 | S1 SAR | +28% | Critical | Disaster day |
| obs-003 | 2026-09-04 | S2 Optical | +14.3% | Elevated | Post-event assessment |

The prevention story: the +8% expansion on 07-23 was the early warning. Had SIREN been monitoring in real time, the watch would have escalated 12 days before the peak, buying lead time to evacuate.
...
| test_audit | 11 | Append-only enforcement, trigger validation |
| test_api | 10 | All API endpoints, human gate, error shapes |
...
- [`docs/PRD.md`](docs/PRD.md) — Product Requirements Document (v4.2)
