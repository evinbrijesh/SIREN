...
| E2 Neural bathymetry | `ml/bathymetry.py`, `train_bathymetry.py`, `ml/bathymetry_dataset.py`, `ml/bathymetry_benchmark.py`, `ml/bathymetry_training_data.py` | 98 (13 architecture + 33 dataset + 25 benchmark + 27 training data) | None — LOO evaluated on 20 real surveyed lakes, all methods fail gate | **Yes — FAILS.** Huggel MAPE 75.6%, regression MAPE 82.1%, neural MAPE 676.4% vs 15% target | No |
...
1. **Bathymetry ground truth (partially resolved — gate evaluated, fails):** 20 surveyed Himalayan glacial lakes (Zhang 2023 + Das 2025, 117k depth points) downloaded with Copernicus DEM GLO30 terrain (6 tiles, 222 MB). The `BathymetryUNet` was trained on real data with leave-one-lake-out evaluation. **Result: all three methods fail the 15% MAPE gate** — Huggel 75.6%, power-law regression 82.1%, neural 676.4%. The neural model is overparameterised for 19 training samples. Huggel remains the best method at this dataset size. Paths forward: more lakes (global compilation has 64 unique), transfer learning from synthetic basins, or simpler terrain-feature regression. See `ml/bathymetry_benchmark.py`, `ml/bathymetry_training_data.py`, `train_bathymetry.py --benchmark` / `--train-real`.
...
- 902 tests pass (677 pre-existing + 47 neural scaffold + 23 S2 optical + 34 fusion dataset + 17 engine wiring + 33 bathymetry dataset + 18 South Lhonak DEM + 25 bathymetry benchmark + 27 bathymetry training data + 1 other).
...
- **E2 bathymetry LOO benchmark evaluated (2026-09-15):** 20 surveyed Himalayan glacial lakes (Zhang 2023 + Das 2025, 117k depth points) with Copernicus DEM GLO30 terrain (6 tiles, 222 MB, no auth from AWS S3). Three volume estimation methods evaluated with leave-one-lake-out: Huggel formula MAPE 75.6%, fitted power-law regression MAPE 82.1%, neural `BathymetryUNet` (100 epochs, 19 training lakes, base_channels=16, n_down=2, grid 128×128) MAPE 676.4%. All three fail the 15% gate. The neural model is overparameterised for 19 samples — it overfits and predicts implausible bed depths. Huggel wins on 13/20 folds and remains the best method at this dataset size. The neural model does beat Huggel on 2 individual lakes (Luggye Tsho 4.7% vs 25.1%, Jialongco 27.4% vs 59.1%), suggesting the architecture can learn useful terrain→depth relationships with more data. See `ml/bathymetry_benchmark.py`, `ml/bathymetry_training_data.py`, `train_bathymetry.py --benchmark` / `--train-real`.
