...
| SAR segmentation | `ml/model.py` + terrain gate | Kuro Siwo IoU 0.62 in-domain; ~6% geographic overlap on Imja (flat-terrain glacier confusion, not slope — measured). Weak-label fine-tune tried 2026-09-17 (`ml/sar_domain_adapt.py`): SCL+lake-mask labels on 49 chips halved glacier water-extent FPs (115k→44k) but change-mask metrics degraded — see note below | More labelled *high-altitude* scenes — single-scene adaptation is insufficient. On-disk sources checked: S1GFloods has no geolocation metadata; FloodPlanet/Nepal is Terai lowland — neither usable (see `docs/DATA_LICENSES.md`) |
...
**Change-mask vs extent semantics (found 2026-09-17):** the ML shadow layer is `water_t1 & ~water_t0` — expansion only. For a *persistent* lake like Imja, a perfectly domain-adapted model detects water at both dates and shows *zero* change — improved per-date detection actively reduces shadow overlap. If the shadow layer is meant to display "model sees water here," it should show water extent (or both layers), not change alone. Current behavior is correct for flood *expansion* but will always under-report for a stable lake.

**License audit:** see `docs/DATA_LICENSES.md` (2026-09-17). Verified: Pleiades DEMs, bathymetry sets, HMAGLOFDB, S1GFloods, Kuro Siwo annotations all CC BY / open. Flagged for public release: RGI redistribution terms, FloodPlanet (PlanetScope is commercial), glacial_lake_2022-2024 provenance, OSM-derived gpkg (ODbL).
