...
| SAR segmentation | `ml/model.py` + terrain gate | Kuro Siwo IoU 0.62 in-domain; ~6% geographic overlap on Imja (flat-terrain glacier confusion, not slope — measured) | Himalayan-region labelled water/flood masks. On-disk sources checked 2026-09-17: S1GFloods has no geolocation metadata; FloodPlanet/Nepal is Terai lowland — neither usable. Options: weak labels from verified Imja masks, or hand-labelled chips vs the clear Nov S2 scene (see `docs/DATA_LICENSES.md`) |
