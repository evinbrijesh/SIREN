...
**Status (2026-09-22): GATE PASSED → advisory-primary.** `ml/train_risk_fusion.py` evaluated the fused scorer on the 887-window corpus (230 dated events + stable/within-lake negatives) under the same spatio-temporal holdout as Tier-2: **mean ROC-AUC 0.835, mean Brier 0.142** vs the deterministic five-factor baseline's **0.504 / 0.187** on identical folds — both gate legs pass. Caveat: the fused model is statistically identical to Tier-2 alone (AUC 0.838) — the stacked fold-honest susceptibility prior and monsoon flag add no measurable accuracy. Wired advisory-first (`risk/learned_fusion.py` → `shadow_evidence["learned_risk_fusion"]` + review reason at p_fused ≥ 0.65); `classify_severity` stays deterministic-authoritative.

**Severity-mapping evaluated (2026-09-22, gate FAILS):** `ml/eval_severity_mapping.py` substituted calibrated `p_fused` for `H` in the unchanged `classify_severity` policy on identical folds (`severity_mapping_eval_report.json`). Learned tiers are ordinally coherent (pooled event rates informational 0.16 → watch 0.43 → elevated 0.48) and dominate at the actionable elevated+ boundary (recall 0.203 / FAR 0.069 vs deterministic 0.000 / 0.000 — the formula under measurable-only inputs flags nothing). But **pooled recall at a 10% false-alarm budget is 0.261, below the declared 0.50 decision-usefulness bar** — the score ranks well but lacks the top-confidence mass to drive a high-recall alert policy. Additional honest limit: the critical tier is unreachable in this eval (needs expansion ≥ 40% or real exposure — no SAR/OSM inputs exist for historical windows), so even a pass would have required a runtime shadow-agreement study. Deterministic `classify_severity` remains authoritative; no severity promotion.
...
| 6.4 | Wire into `risk/fusion.py` | `classify_severity` uses learned score when promoted; deterministic stays fallback — **EVALUATED, NOT PROMOTED: advisory-first wiring (`risk/learned_fusion.py` + shadow evidence + review reason); severity-mapping gate failed (recall 0.26 @ 10% FAR < 0.50)** |
