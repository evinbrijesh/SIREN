...
def build_features(samples: pd.DataFrame) -> pd.DataFrame:
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "dataset": out_display,
        "weather_source": (
            "Open-Meteo archive (ERA5/ERA5-Land reanalysis) — "
        ...
        **meta,
        "known_limitations": [
            (
                "ERA5 precipitation is reanalysis, not IMERG satellite "
                "precip — 9–11 km grid smooths convective extremes"
            ),
            (
                "negative windows may coincide with unrecorded events "
                "(label noise)"
            ),
            "events before 1940 excluded (archive coverage)",
